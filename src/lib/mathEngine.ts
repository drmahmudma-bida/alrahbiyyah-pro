 ...
 * THE AL-RAHBIYYAH FRAMEWORK - V7.1 (THE MASTERPIECE)
 * Complete Multi-Madhab Engine + Dynamic Muqasamah (Grandfather vs. Siblings)
 * Integrated with Dual-Authority Proofs Database
 */
...
export function calculateUltimateRahbiyyah(heirs: any, selectedMadhab: string = 'shafii', netAssetValue: number = 100000) {
  ...
  if (heirs.unborn_foetus > 0) {
    ...
    // We push the escrow immediately
    results.push({ 
  ...
  // --- NEW: DYNAMIC PROOF FETCHER ---
  // This reads the heir's name and fetches the precise Arabic texts from proofsData.ts
  const getProofObj = (key: string, rule: string) => {
    ...
    if (rule === 'mushtarakah' && proofsDatabase.complex_cases?.mushtarakah) return proofsDatabase.complex_cases.mushtarakah;
    ...
    if (key.includes('Full Brothers') || key.includes('Full Siblings')) return proofsDatabase.full_brothers.general;
    if (key.includes('Full Sisters')) return proofsDatabase.full_sisters.general;
    if (key.includes('Maternal')) return proofsDatabase.maternal_siblings.general;
    if (key.includes('Paternal Grandfather')) return proofsDatabase.father.general; 
    if (key.includes('Paternal Grandmother')) return proofsDatabase.paternal_grandmother.general;
    if (key.includes('Maternal Grandmother')) return proofsDatabase.maternal_grandmother.general;
    if (key.includes('Grandmothers')) return proofsDatabase.grandmothers_shared.general;
    if (key.includes('Son\'s Daughters')) return proofsDatabase.granddaughters.general;
    if (key.includes('Paternal Brothers')) return proofsDatabase.paternal_brothers.general;
    if (key.includes('Paternal Sisters')) return proofsDatabase.paternal_sisters.general;
  ...
  // Helper to convert decimals to readable fractions like '1/4'
  const toFractionString = (decimal: number) => {
    ...
    return "Calculated"; // For Awl, Radd, and complex Muqasamah math
  };

  // --- UPGRADED FORMAT OUTPUT (Connects Math to the Dashboard UI) ---
  for (let [heirKey, val] of Object.entries(shares)) {
    if (val > 0) {
      ...
      results.push({ 
         ...
         amount: netAssetValue * val,  // Calculates exact money!
         fraction: toFractionString(val),
         rule: proof.ruleTitle,
         quranProof: proof.quran,
         // This is the magic that switches the Arabic based on the school!
